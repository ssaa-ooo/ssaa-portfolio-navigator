...
import { Settings2, RefreshCw, Save, X, Compass, Star, Edit3, Lightbulb, Users } from 'lucide-react';
...
export default function SSAANavigator() {
  ...
  const renderSlider = (label: string, key: string, min = 1, max = 5) => (
    <div className="mb-6">
      ...
      <input type="range" min={min} max={max} value={tempScores[key]} onChange={e => setTempScores({...tempScores, [key]: parseInt(e.target.value)})} className="w-full h-1.5 bg-slate-100 rounded-full appearance-none cursor-pointer accent-blue-600" />
      {max === 5 && <p className="text-[9px] text-blue-500 font-bold mt-2 italic">{settings[`Score_${tempScores[key]}_Def`]}</p>}
    </div>
  );
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="flex items-center gap-8">
                ...
                <div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-black leading-tight italic tracking-tight">「{settings.Vision_Statement || "ビジョンを定義してください"}」</h2>
                </div>
              </div>
              <button onClick={() => setIsEditingVision(true)} className="flex items-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 rounded-2xl border border-white/10 transition-all text-sm font-bold"><Edit3 className="w-4 h-4" /> Edit Vision</button>
            </div>
          ) : (
            <div className="relative z-10 animate-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-3 mb-6"><Lightbulb className="w-6 h-6 text-yellow-400" /><h3 className="font-black text-xl tracking-tight">Vision Crafting Support</h3></div>
              <textarea value={tempVision} onChange={(e) => setTempVision(e.target.value)} className="w-full bg-white/10 border-2 border-white/20 rounded-3xl p-6 text-xl font-bold focus:border-white/50 outline-none min-h-[120px]" />
              <div className="flex gap-3 mt-6">
                <button onClick={() => saveSetting('Vision_Statement', tempVision)} disabled={isSaving} className="px-8 py-4 bg-white text-blue-900 rounded-2xl font-black">{isSaving ? "Saving..." : "Save Vision"}</button>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-7 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm h-[650px]">
          <h2 className="text-2xl font-black mb-10 text-slate-800 flex items-center gap-3">
            ...
            Selection Orbit (Asset Volume Map)
          </h2>
              ...
              {editingId === p.id ? (
                <div className="pt-6 border-t border-slate-50 space-y-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    {renderSlider('Asset Volume (Weight)', 'z', 10, 200)}
                    <div className="mt-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Lead Person</label>
                      <input type="text" value={tempScores.lead} onChange={e => setTempScores({...tempScores, lead: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-2 text-sm font-bold outline-none focus:border-blue-400" />
                    </div>
                  </div>
                </div>
              ) : (
                <div className="flex gap-8 border-t pt-4 border-slate-50">
                  <div><p className="text-[10px] font-black text-slate-300 uppercase mb-1">Sync</p><p className="text-xl font-black text-slate-700">{Math.round(p.x)}%</p></div>
                  <div><p className="text-[10px] font-black text-slate-300 uppercase mb-1">Velocity</p><p className="text-xl font-black text-slate-700">{Math.round(p.y)}%</p></div>
                  <div><p className="text-[10px] font-black text-slate-300 uppercase mb-1">Asset Vol</p><p className="text-xl font-black text-blue-600">{p.z}</p></div>
                </div>
              )}
            </div>
          ))}
